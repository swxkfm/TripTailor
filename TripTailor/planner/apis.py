...
from prompts import planner_agent_prompt, cot_planner_agent_prompt, react_planner_agent_prompt,reflect_prompt,react_reflect_planner_agent_prompt, REFLECTION_HEADER, expand_prompt
...
class Planner:
    def __init__(self,
        ...
        if model_name in  ['mistral-7B-32K']:
            ...
            self.llm = ChatOpenAI(model_name=model_name, temperature=0, max_tokens=4096, openai_api_key=OPENAI_API_KEY, openai_api_base=OPENAI_BASE_URL)
...
class ReactPlanner:
    ...
    def is_halted(self) -> bool:
        return ((self.curr_step > self.max_steps) or (
                    len(self.enc.encode(self._build_agent_prompt())) > 30000)) and not self.finished
...
class ReactReflectPlanner:
    ...
    def __init__(self,
        ...
        if model_name in ['gemini']:
            ...
            self.reflect_llm = ChatOpenAI(model_name=model_name, temperature=0, max_tokens=1024, 
                                          openai_api_key=OPENAI_API_KEY,openai_api_base=OPENAI_BASE_URL,
                                          model_kwargs={"stop": ["Action","Thought","Observation,'\n"]})
        self.model_name = model_name
    ...
    def run(self, text, query, id, reset = True) -> None:
    ...
    def step(self) -> None:
        ...
        if action_type == 'CostEnquiry':
            try:
                ...
                if type(input_arg) != dict:
                    raise ValueError('The sub plan can not be parsed into json format, please check. Only one day plan is supported.')
                observation = f'Cost: {self.env.run(input_arg, self.id)}'
            except SyntaxError:
                observation = f'The sub plan can not be parsed into json format, please check.'
            except ValueError as e:
                observation = str(e)
    ...
    def is_halted(self) -> bool:
        return ((self.curr_step > self.max_steps) or (
                    len(self.enc.encode(self._build_agent_prompt())) > 20000)) and not self.finished
