...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.output_dir is not None:
            print_rank_0(f'saving model epoch_{epoch+1} ...', args.global_rank)
            rm_model_save = convert_lora_to_linear_layer(rm_model)

            if args.global_rank == 0:
                save_hf_format(rm_model_save, tokenizer, args, sub_folder=f"epoch_{epoch+1}")
            if args.zero_stage == 3:             
                # for zero stage 3, each gpu only has a part of the model, so we need to save the model on each gpu by using DS-Engine
                output_dir_tmp = os.path.join(args.output_dir, f"epoch_{epoch+1}")
